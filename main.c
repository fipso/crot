...
int initBackgroundVideo(BackgroundVideo *bg, const char *filename) {
    ...
    const char* hw_decoder_names[] = {
        "h264_vaapi",    // AMD VAAPI H.264 decoder
    ...
    if (strstr(codec->name, "vaapi")) {
        ...
        if (avcodec_open2(bg->codec_ctx, codec, &opts) < 0) {
            printf("Warning: Could not open hardware decoder, trying software fallback\n");
            av_dict_free(&opts);
    ...
    bg->sws_ctx = sws_getContext(
        src_width, src_height, bg->codec_ctx->pix_fmt,
...
int getBackgroundFrame(BackgroundVideo *bg, double target_time, uint8_t *rgba_buffer) {
    ...
    if (last_pts == -1 || abs((int)(target_pts - last_pts)) > 30) { // 30 frame tolerance
        if (av_seek_frame(bg->fmt_ctx, bg->stream_index, target_pts, AVSEEK_FLAG_BACKWARD) >= 0) {
            ...
            if (avcodec_send_packet(bg->codec_ctx, bg->pkt) >= 0) {
                while (avcodec_receive_frame(bg->codec_ctx, bg->frame) >= 0) {
                    ...
                    if (fabs(frame_time - target_time) < 0.5) {
                        ...
                        sws_scale(bg->sws_ctx, 
                                 (const uint8_t * const*)bg->frame->data, bg->frame->linesize,
                                 0, src_height,
                                 temp_data, temp_linesize);
                        ...
                        for (int y = 0; y < HEIGHT; y++) {
                            ...
                            if (src_y >= 0 && src_y < scaled_height) {
                                uint8_t *src_line = temp_buffer + (src_y * scaled_width + offset_x) * 4;
...
void cleanupBackgroundVideo(BackgroundVideo *bg) {
    if (bg->sws_ctx) sws_freeContext(bg->sws_ctx);
    if (bg->frame) av_frame_free(&bg->frame);
    if (bg->pkt) av_packet_free(&bg->pkt);
    if (bg->codec_ctx) avcodec_free_context(&bg->codec_ctx);
    if (bg->fmt_ctx) avformat_close_input(&bg->fmt_ctx);
}

// Load audio files for mixing
int loadAudioFiles(const char* projectId, AudioFile** audioFiles, int* audioCount) {
    char audioDir[256];
    ...
    *audioCount = 0;
    ...
    for (int fileIdx = 0; fileIdx < fileCount; fileIdx++) {
        char filePath[512];
        snprintf(filePath, sizeof(filePath), "%s/%s", audioDir, entries[fileIdx]->d_name);
        ...
        if (avformat_open_input(&af->fmt_ctx, filePath, NULL, NULL) >= 0) {
            if (avformat_find_stream_info(af->fmt_ctx, NULL) >= 0) {
                ...
                for (unsigned int i = 0; i < af->fmt_ctx->nb_streams; i++) {
                    if (af->fmt_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                        af->stream_index = i;
                ...
                if (af->stream_index != -1) {
                    // Initialize decoder
                    const AVCodec *codec = avcodec_find_decoder(af->audio_stream->codecpar->codec_id);
                    if (codec) {
                        af->codec_ctx = avcodec_alloc_context3(codec);
                        if (avcodec_parameters_to_context(af->codec_ctx, af->audio_stream->codecpar) >= 0) {
                            if (avcodec_open2(af->codec_ctx, codec, NULL) >= 0) {
                                ...
                                if (af->swr_ctx) {
                                    av_opt_set_chlayout(af->swr_ctx, "in_chlayout", &af->codec_ctx->ch_layout, 0);
                                    av_opt_set_int(af->swr_ctx, "in_sample_rate", af->sample_rate, 0);
                                    av_opt_set_sample_fmt(af->swr_ctx, "in_sample_fmt", af->codec_ctx->sample_fmt, 0);
                                    
                                    AVChannelLayout stereo_layout = AV_CHANNEL_LAYOUT_STEREO;
                                    av_opt_set_chlayout(af->swr_ctx, "out_chlayout", &stereo_layout, 0);
                                    av_opt_set_int(af->swr_ctx, "out_sample_rate", 44100, 0);
                                    av_opt_set_sample_fmt(af->swr_ctx, "out_sample_fmt", AV_SAMPLE_FMT_FLTP, 0);
                                    ...
                                    if (swr_init(af->swr_ctx) < 0) {
                                        ...
                                        int estimated_samples = 44100 * 5; // 5 seconds stereo at 44.1kHz
                                        ...
                                        if (af->stereo_buffer) {
                                            ...
                                            while (av_read_frame(af->fmt_ctx, af->pkt) >= 0) {
                                                if (af->pkt->stream_index == af->stream_index) {
                                                    if (avcodec_send_packet(af->codec_ctx, af->pkt) >= 0) {
                                                        while (avcodec_receive_frame(af->codec_ctx, af->frame) >= 0) {
                                                            uint8_t **out_data = NULL;
                                                            int out_linesize;
                                                            int out_samples = av_rescale_rnd(af->frame->nb_samples, 44100, af->sample_rate, AV_ROUND_UP);
                                                            
                                                            if (av_samples_alloc_array_and_samples(&out_data, &out_linesize, 2, out_samples, AV_SAMPLE_FMT_FLTP, 0) >= 0) {
                                                                int converted = swr_convert(af->swr_ctx, out_data, out_samples,
                                                                                           (const uint8_t**)af->frame->data, af->frame->nb_samples);
                                                                
                                                                if (converted > 0 && af->buffer_samples + converted < estimated_samples) {
                                            ...
                                            printf("Preloaded %d samples (%.2f seconds)\n", af->buffer_samples, (float)af->buffer_samples / 44100.0f);
                                        }
                                        
                                        // Reset to beginning for potential future use
                                        av_seek_frame(af->fmt_ctx, af->stream_index, 0, AVSEEK_FLAG_BACKWARD);
...
int main(int argc, char* argv[]) {
    ...
    av_dict_set(&encoder_opts, "thread_type", "slice+frame", 0);  // Enable both slice and frame threading
    av_dict_set(&encoder_opts, "x264-params", "aq-mode=0:me=dia:subme=1:ref=1:analyse=none:trellis=0:no-fast-pskip=0:8x8dct=0:sliced-threads=1", 0);
...
int frame_idx = 0;
    ...
    for (int i = 0; i < captionCount && i < audioFileCount; i++) {
        if (currentTime >= captions[i].startTime && currentTime <= captions[i].endTime) {
            ...
            if (!af->stereo_buffer || af->buffer_samples == 0) continue;
    ...
    memcpy(audio_buffer_left + audio_buffer_len, temp_left, samples_this_frame * sizeof(float));
    memcpy(audio_buffer_right + audio_buffer_len, temp_right, samples_this_frame * sizeof(float));
    ...
    while (audio_buffer_len >= frame_size) {
        ...
        memcpy(audio_frame->data[1], audio_buffer_right, frame_size * sizeof(float));
        ...
        memmove(audio_buffer_left, audio_buffer_left + frame_size, (audio_buffer_len - frame_size) * sizeof(float));
        memmove(audio_buffer_right, audio_buffer_right + frame_size, (audio_buffer_len - frame_size) * sizeof(float));
        ...
        if (renderMode && frame_idx % 60 == 0) { // Every second
            ...
            printf("Progress: %.1f%% (%d/%d frames) - %.1f fps\n", 
                   progress, frame_idx, FRAME_COUNT, 60.0f / GetFrameTime());
        }
    }
    ...
    free(audio_buffer_right);
            ...
            for (int i = 0; i < audioFileCount; i++) {
                if (audioFiles[i].stereo_buffer) free(audioFiles[i].stereo_buffer);
                if (audioFiles[i].swr_ctx) swr_free(&audioFiles[i].swr_ctx);
                if (audioFiles[i].codec_ctx) avcodec_free_context(&audioFiles[i].codec_ctx);
                if (audioFiles[i].frame) av_frame_free(&audioFiles[i].frame);
                if (audioFiles[i].pkt) av_packet_free(&audioFiles[i].pkt);
                if (audioFiles[i].fmt_ctx) avformat_close_input(&audioFiles[i].fmt_ctx);
            }
            free(audioFiles);
        }
